...
class CppGenerator : public BaseGenerator {
 ...
 private:
  ...
  void GenBuilders(const StructDef &struct_def) {
    ...
    code_ += "    const flatbuffers::uoffset_t end = fbb_.EndTable(start_);";
      ...
      for (auto it = struct_def.fields.vec.begin();
        ...
        if (!field.deprecated) {
          ...
          if (field.value.type.base_type == BASE_TYPE_STRING) {
            code_ +=
                ",\n      {{FIELD_NAME}} ? "
                "_fbb.CreateString({{FIELD_NAME}}) : flatbuffers::Offset<flatbuffers::String>(0)\\";
            ...
            code_ += "(*{{FIELD_NAME}}) : flatbuffers::Offset<flatbuffers::Vector<" + GenTypeWire(vtype, "", false) + " > >(0)\\";
          } else {
            code_ += ",\n      {{FIELD_NAME}}\\";
          }
        }
...
bool GenerateBCC(const Parser &parser, const std::string &path,
                 const std::string &file_name) {
  bcc::CppGenerator generator(parser, path, file_name);
